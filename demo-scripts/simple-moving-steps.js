...
// Add a blue square that moves across the screen
gameCanvas.addDrawing(function ({ ctx, width, height, stepTime }) {
  // stepTime tells us how long it's been since the last frame we drew (in ms);
  // We will use it to move the square to the right at 100 pixels per second
  ...
  if (x > width) {
    ...
    y += 30; // move down a row each time it goes off screen
  }
  if (y > height) {
